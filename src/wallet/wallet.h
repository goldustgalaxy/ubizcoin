...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 UTC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zUTC send
enum ZerocoinSpendStatus {
    ZUTC_SPEND_OKAY = 0,                            // No error
    ZUTC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZUTC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZUTC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZUTC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZUTC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZUTC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZUTC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZUTC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZUTC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZUTC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZUTC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZUTC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZUTC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZUTC_SPENT_USED_ZUTC = 14,                      // Coin has already been spend
    ZUTC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZUTC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZUTCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzUTCTracker> zpivTracker;
    ...
    void setZWallet(CzUTCWallet* zwallet);
    CzUTCWallet* getZWallet();
    ...
    /// Get 10000 UTC output and keys which can be used for the Masternode
    ...
    /** zUTC reset */
    boost::signals2::signal<void()> NotifyzUTCReset;
