...
// Copyright (c) 2016-2020 The UbizCoin developers
...
public:
    ...
    bool WriteZUTCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZUTCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZUTCSeed_deprecated(uint256& seed);
    bool EraseZUTCSeed();
    bool EraseZUTCSeed_deprecated();

    bool WriteZUTCCount(const uint32_t& nCount);
    bool ReadZUTCCount(uint32_t& nCount);
