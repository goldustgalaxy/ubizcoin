// Copyright (c) 2018-2020 The UbizCoin developers
...
CzUTCTracker::CzUTCTracker(std::string strWalletFile)
{
...
CzUTCTracker::~CzUTCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzUTCTracker::Init()
{
...
bool CzUTCTracker::Archive(CMintMeta& meta)
{
...
bool CzUTCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzUTCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzUTCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzUTCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzUTCTracker::GetSerialHashes()
{
...
CAmount CzUTCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzUTCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzUTCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzUTCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzUTCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzUTCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzUTCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzUTCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzUTCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzUTCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzUTCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzUTCWallet* zUTCWallet)
{
    bool iszUTCWalletInitialized = (NULL != zUTCWallet);
    CMintMeta meta;
    ...
    if (! iszUTCWalletInitialized)
        zUTCWallet = new CzUTCWallet(strWalletFile);
    meta.isSeedCorrect = zUTCWallet->CheckSeed(dMint);
    if (! iszUTCWalletInitialized)
        delete zUTCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzUTCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzUTCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzUTCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzUTCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzUTCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzUTCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzUTCWallet* zUTCWallet = new CzUTCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zUTCWallet);
        }
        delete zUTCWallet;
