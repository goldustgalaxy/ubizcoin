...
CzUTCWallet::CzUTCWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZUTCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZUTCSeed_deprecated()) {
                    LogPrintf("%s: Updated zUTC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzUTCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZUTCCount(nCountLastUsed);
    else if (!walletdb.ReadZUTCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzUTCWallet::Lock()
{
    seedMaster = 0;
}

void CzUTCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzUTCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZUTC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzUTCWallet::LoadMintPoolFromDB()
{
...
void CzUTCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzUTCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzUTCWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzUTCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZUTC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzUTCWallet::SeedToZUTC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzUTCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzUTCWallet::GenerateDeterministicZUTC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzUTCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzUTCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzUTCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
