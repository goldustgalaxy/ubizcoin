...
#include "qt/ubizcoin/send.h"
#include "qt/ubizcoin/forms/ui_send.h"
#include "qt/ubizcoin/addnewcontactdialog.h"
#include "qt/ubizcoin/qtutils.h"
#include "qt/ubizcoin/sendchangeaddressdialog.h"
#include "qt/ubizcoin/optionbutton.h"
#include "qt/ubizcoin/sendconfirmdialog.h"
#include "qt/ubizcoin/myaddressrow.h"
#include "qt/ubizcoin/guitransactionsutils.h"
...
SendWidget::SendWidget(UbizCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (UTC)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("UbizCoin address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 UTC");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_UTC : AskPassphraseDialog::Context::Send_zUTC, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zUTC is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zUTC transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZUTC_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zUTC require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zUTC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isUTC){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zUTC in your balance to select."));
        }
    }
}
...
void SendWidget::onUTCSelected(bool _isUTC){
    isUTC = _isUTC;
    setCssProperty(coinIcon, _isUTC ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
