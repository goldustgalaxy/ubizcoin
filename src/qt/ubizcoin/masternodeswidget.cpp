...
#include "qt/ubizcoin/forms/ui_masternodeswidget.h"
#include "qt/ubizcoin/qtutils.h"
#include "qt/ubizcoin/mnrow.h"
#include "qt/ubizcoin/mninfodialog.h"

#include "qt/ubizcoin/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(UbizCoinGUI *parent) :
    PWidget(parent),
    ...
    if (dialog->exportMN){
        if (ask(tr("Remote Masternode Data"),
                tr("You are just about to export the required data to run a Masternode\non a remote server to your clipboard.\n\n\n"
                   "You will only have to paste the data in the ubizcoin.conf file\nof your remote server and start it, "
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= CollateralRequired(chainActive.Height())){
            ...
            // kind of like "...node, %i UTC requited").arg(qYourCollateralResultVariable),
            // to nicely print required collateral amount.
            inform(tr("Not enough balance to create a masternode")); //, 10,000 UTC required."));
            return;
        }
        showHideOp(true);
