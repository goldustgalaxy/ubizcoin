...
#include "qt/ubizcoin/privacywidget.h"
#include "qt/ubizcoin/forms/ui_privacywidget.h"
#include "qt/ubizcoin/qtutils.h"
#include "guiutil.h"
#include "qt/ubizcoin/denomgenerationdialog.h"
#include "qt/ubizcoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(UbizCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zUTC anonymizes your UTC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zUTC or convert back to UTC"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 UTC ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zUTC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zUTC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zUTC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zUTC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zUTC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zUTC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zUTC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zUTC");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zUTC Movements"));
    ...
    // Only Convert to UTC enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzUTC->setTitleClassAndText("btn-title-grey", "Total 0 zUTC");
    ui->btnTotalzUTC->setSubTitleClassAndText("text-subtitle", "Show denominations of zUTC owned.");
    ui->btnTotalzUTC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select UTC outputs to mint into zUTC.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zUTC");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzUTC, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zUTC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of UTC to mint into zUTC"));
    }else{
        btnText = tr("Convert back to UTC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zUTC to convert back into UTC"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzUTC->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzUTC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zUTC is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zUTC, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zUTC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zUTC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zUTC converted back to UTC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any UTC to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzUTC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
