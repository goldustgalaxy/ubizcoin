                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zUTC ought to be enough for anybody." - Bill Gates, 2017
    ui->zUTCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzUTCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzUTC->setEnabled(false);
        ui->pushButtonMintzUTC->setToolTip(tr("zUTC is currently disabled due to maintenance."));

        ui->pushButtonSpendzUTC->setEnabled(false);
        ui->pushButtonSpendzUTC->setToolTip(tr("zUTC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zUTCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzUTC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zUTC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zUTC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zUTC in ") +
...
void PrivacyDialog::on_pushButtonSpendzUTC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zUTC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zUTC
        sendzUTC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zUTC
    sendzUTC();
}

void PrivacyDialog::on_pushButtonZUtcControl_clicked()
{
    ZUtcControlDialog* zUtcControl = new ZUtcControlDialog(this);
    zUtcControl->setModel(walletModel);
    zUtcControl->exec();
}

void PrivacyDialog::setZUtcControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzUtcSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzUTC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Ubizcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zUTCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zUTC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " UTC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zUTC</b>";
    ...
    // use mints from zUtc selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZUtcControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZUtcControlDialog::GetSelectedMints();
    }
    ...
    // Clear zutc selector in case it was used
    ZUtcControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zUtc Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zUTC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zUTC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zUTC "));
    ui->labelzUTCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzUTCSyncStatus->setVisible(fShow);
}
