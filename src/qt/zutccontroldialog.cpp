...
#include "ui_zutccontroldialog.h"
...
std::list<std::string> ZUtcControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZUtcControlDialog::listMints;

ZUtcControlDialog::ZUtcControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZUtcControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZUtcControlDialog::~ZUtcControlDialog()
{
    delete ui;
}

void ZUtcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZUtcControlDialog::updateList()
{
...
void ZUtcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZUtcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZUtc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZUtcControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZUtcControlDialog::GetSelectedMints()
{
...
void ZUtcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
