...
// Copyright (c) 2015-2019 The UbizCoin developers
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("zUTCsupply", zpivObj));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZUTC()) ? "Not available" : std::to_string(
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in ubizcoin\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of ubizcoin addresses\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZUTC_FAILED_ACCUMULATOR_INITIALIZATION);
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (txin.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (isPublicSpend) {
                        ...
                        if (!ZUTCModule::parseCoinSpend(txin, tx, prevOut, publicSpend)) {
